...
class DeleteExecutor : public AbstractExecutor {
 ...
 private:
  ...
  std::shared_ptr<TableInfo> table_info_;            // 目标表的元信息
  std::vector<std::shared_ptr<IndexInfo>> indexes_;  // 表关联的所有索引
  size_t deleted_count_;                             // 已删除的行数
  bool is_finished_;                                 // 标记是否已返回删除行数
};
}  // namespace bustub
